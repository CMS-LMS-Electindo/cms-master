        ...
        <table id="tabel" class="table table-row-dashed table-row-gray-300 align-middle gs-0 gy-4">
          ...
          <thead>
            <tr class="fw-bolder text-muted">
              ...
              <th class="min-w-150px">Config Mata Kuliah</th>
        ...
        <form id="kt_modal_new_target_form" class="form" action="#">
          ...
          <div class="d-flex flex-column mb-8 fv-row">
            ...
            <label class="d-flex align-items-center fs-6 fw-bold mb-2">
              <span class="required" for="namaSemester">Name of Semester</span>
              <i class="fas fa-exclamation-circle ms-2 fs-7" data-bs-toggle="tooltip" title="Specify a target name for future usage and reference"></i>
            </label>
            <!--end::Label-->
            <input type="text" class="form-control form-control-solid" placeholder="Enter Name of Semester" name="namaSemester" id="namaSemester" />
          </div>
          <!--end::Input group-->
          <!--begin::Input group-->
...
<script>
  ...
  function edit(id) {
    ...
    $.get("{{ url('semester') }}" +'/' + id +'/edit', function (data) {
        ...
        var sp = data.tanggal_lahir.split("-");
        $('#tanggal').val(sp[2]+"-"+sp[1]+"-"+sp[0]);
    })
  }
  function hapus(id) {
    Swal.fire({
  ...
  $(".make-user").click(function () {
    Swal.fire({
        title: 'Apakah anda yakin akan membuatkan user login semua semester?',
        text: "",
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#f00',
        ...
        if (result.value) {
            $(".make-user").html("<i class='fas fa-sync-alt mt-0 fa-spin'></i> Loading");
            $(".make-user").addClass('bg-warning');
            $(".make-user").removeClass("btn-outline-white");
                $.ajax({
                type: "POST", data: {'_token': '{{ csrf_token() }}'},
                url: "{{ url('make-student-login') }}",
                // contentType: false,
                // cache:false,
                // processData: false,
                dataType:"json",
                success: function(response) {
                  $(".make-user").html("<i class='fas fa-sync-alt mt-0 '></i> ");
                  $(".make-user").removeClass('bg-warning');
                  $(".make-user").addClass("btn-outline-white");
                    if (response.status==true) {
                    toastr.success(response.message, 'Terhapus', {timeOut: 5000});
                    } else{
                    toastr.error(response.message, 'Gagal', {timeOut: 5000});
